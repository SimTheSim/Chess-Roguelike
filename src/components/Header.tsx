...
export const Header = () => {
    ...
    `} id="page-header">
      ...
      <div className="flex items-center gap-6">
        {status === 'start' && mpRoomCode && (
          <button
            onClick={() => {
              if (mpSocket) {
                const clientId = getClientId();
                mpSocket.emit('reconnect_to_room', { code: mpRoomCode, clientId }, (res: any) => {
        ...
        {status !== 'start' && gameMode === 'online' && (
          <button
            onClick={() => {
              mpSocket?.emit('leave_room', { code: mpRoomCode });
              mpSocket?.disconnect();
